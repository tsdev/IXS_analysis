function [x, y, err, xlab, ylab,monitor]=specload(filespec)
%
%function [x, y, err, xlab, ylab,monitor]=specload(file)
%
...
% ACWalters 6.11.09: For ID28 at the ESRF, reading IXS data from spec file
%
%
...
% analysers array contains the numbers of the working analysers for ID28
analysers=[1 2 3 5 6 7 8 9];

%---- Set default column names and flags
scan='';

for i=analysers
    eval(['xname(i)=A' num2str(i) ' Resistance;']);
end

for i=analysers
    eval(['yname(i)=A' num2str(i) ' Intensity;']);
end
...
x(analysers)=[]; y(analysers)=[]; e(analysers)=[]; ylab(analysers)=''; xlab=''; monitor = [];
...
for j=analysers
    eval(['i=strmatch(''X(' num2str(j) ')='',fspec);']);
    if ~isempty(i)
        xname=deblank(fspec(i(end),3:nchars)); end
end

for j=analysers
    eval(['i=strmatch(''Y(' num2str(j) ')='',fspec);']);
    if ~isempty(i)
...
for j=analysers
    eval(['i=strmatch(''E(' num2str(j) ')='',fspec);']);
    if ~isempty(i)
        yname=deblank(fspec(i(end),3:nchars)); end
end
