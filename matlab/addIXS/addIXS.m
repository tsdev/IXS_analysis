function varargout = addIXS(varargin)
% ADDIXS M-file for addIXS.fig
%      ADDIXS, by itself, creates a new ADDIXS or raises the existing
%      singleton*.
%
%      H = ADDIXS returns the handle to a new ADDIXS or the handle to
%      the existing singleton*.
%
%      ADDIXS('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ADDIXS.M with the given input arguments.
%
%      ADDIXS('Property','Value',...) creates a new ADDIXS or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before addIXS_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to addIXS_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help addIXS

% Last Modified by GUIDE v2.5 15-May-2010 16:29:20
%
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @addIXS_OpeningFcn, ...
                   'gui_OutputFcn',  @addIXS_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before addIXS is made visible.
function addIXS_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to addIXS (see VARARGIN)

% Choose default command line output for addIXS
...
handles.max_num_scans=25;

% Set up zero array for the T_shifts to be applied to the data
handles.T_shifts = zeros(handles.max_num_scans,length(handles.analysers));

% Initialise hints
set(handles.hints,'string','Choose SPEC file/s and scan number/s');

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes addIXS wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = addIXS_OutputFcn(hObject, eventdata, handles) 
...
[temp,temp2,filterindex] = uigetfile({'*.*','All Files (*.*)'},'Choose SPEC file','MultiSelect', 'on');

if ~isequal(temp,0)
    eval(['handles.specfile(' num2str(spec_number) ')= {temp};']);
    eval(['handles.datadir(' num2str(spec_number) ')= {temp2};']);
...
% --------------------------------------------------------------------
function file3_find_Callback(hObject, eventdata, handles)
% hObject    handle to load_tag (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% --------------------------------------------------------------------
function file4_find_Callback(hObject, eventdata, handles)
% hObject    handle to load_tag (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% --------------------------------------------------------------------
function file5_find_Callback(hObject, eventdata, handles)
% hObject    handle to load_tag (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% --- Executes on button press in read_scans.
function read_scans_Callback(hObject, eventdata, handles)
% hObject    handle to read_scans (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% for i=1:length(handles.specfile)
%     copyfile([handles.datadir cell2mat(handles.specfile(i))],cell2mat(handles.specfile(i)),'f');
% end
...
% Find difference between maximum possible number of scans and total scans
% read
difference=handles_out.max_num_scans-total_scan_nums;

% Make array with 1 if data row in GUI is to be used and 0 if not
used_data_posns=[handles_out.state;zeros(difference,1)];

% Reduce visibility of GUI so that only buttons which are useful can be
% seen
for i=1:handles_out.max_num_scans
    if used_data_posns(i)
        eval(['set(handles_out.data' num2str(i) '_tag,''Visible'',''on'');']);
        eval(['set(handles_out.T_data' num2str(i) '_tag,''Visible'',''on'');']);
        eval(['set(handles_out.slider_data' num2str(i) ',''Visible'',''on'');']);
        eval(['set(handles_out.fit_data' num2str(i) ',''Visible'',''on'');']);
        eval(['set(handles_out.text' num2str(i) ',''Visible'',''on'');']);
    else
        eval(['set(handles_out.data' num2str(i) '_tag,''Visible'',''off'');']);
        eval(['set(handles_out.T_data' num2str(i) '_tag,''Visible'',''off'');']);
        eval(['set(handles_out.slider_data' num2str(i) ',''Visible'',''off'');']);
        eval(['set(handles_out.fit_data' num2str(i) ',''Visible'',''off'');']);
        eval(['set(handles_out.text' num2str(i) ',''Visible'',''off'');']);
    end
end

% add a forward slash to the current directory (causes problem on Windows
% machines)
% currdir=[pwd '/'];

% Load scans from SPEC file
% handles_out.data = ID28_loaddata(currdir,handles.specfile,handles_out.scan_numbers);

% Load scans from SPEC file (Ione used to normalise data)
[handles_out.data] = ID28_loaddata(handles.datadir,handles.specfile,handles_out.scan_numbers);

% Load scans from SPEC file (Imirr used to normalise data)
% [handles_out.data] = ID28_loaddata_Imirr(handles.datadir,handles.specfile,handles_out.scan_numbers);
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDuqux8m35ATA)
...
eval(['set(handles_out.T_data' num2str(data_number) '_tag,''string'',num2str(handles.T_shifts(data_number,handles.chosen_analyser),''%.4g''));']);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% make automatic filename
handles.filename = ID28_make_filename(handles.specfile,handles.scan_numbers,handles.state,handles.chosen_analyser);

% update filename_tag with automatic choice
set(handles.filename_tag,'string',handles.filename)
...
ID28_writedata_GUI(handles.datadir,handles.specfile,handles.scan_numbers,handles.state,handles.chosen_analyser,handles.Xs,handles.Ys,handles.Es,handles.Is_sum,handles.Ns_sum,handles.Es_sum,handles.filename);
...
% Hints: get(hObject,'String') returns contents of hints as text
%        str2double(get(hObject,'String')) returns contents of hints as a double


% --- Executes during object creation, after setting all properties.
function hints_CreateFcn(hObject, eventdata, handles)
% hObject    handle to hints (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
