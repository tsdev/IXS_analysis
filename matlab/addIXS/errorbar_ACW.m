function hh = errorbar_ACW(varargin)
...
% Improved form of error bars: ACWalters 12/10/2008
% make width of errorbar the average distance from one datapoint
% to the next
tee = (max(x)-min(x))/(2*length(x));
xl = x - tee;
xr = x + tee;
ytop = y + u;
ybot = y - l;
