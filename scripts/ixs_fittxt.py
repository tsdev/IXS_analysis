...
import sys, traceback, string, os, time, h5py, tempfile
import numpy as np
from types import *
...
#--------------------------------------------------------------------------------------------------------
	...
	def Ft_I(self,p, E, interpolation=1, mask=None, convolution=1): 
		...
		icontribution=0
		ipar=0#Number of parameters of the whole model = sum of the nPars of all contributions
		result=0.0# This is the final model
		for contribution in self.params_and_functions.shapes:
			...
			if icontribution==0:#Firstly, this is for elastic line
...
def interactive_GUI_get_init_peak_params(E,A):
	...
	if not REPLAY==0:
		...
		noel = zeroinv(E)
		if noel:
			print "Select ONLY the anti-stokes maxima for each excitation."
			print "Close the plot to start the fit."
		else:
			print "Pick first the maximum of the elastic line."
			print "Select the anti-stokes maxima for each excitation."
			print "Close the plot to start the fit."
...
def build_param_name_dict(params_and_functions):
	...
	res={}
	for contribution in params_and_functions.shapes:
		npars = contribution.nofMyParams()#3
		parnames = contribution.parNames()# ["Center", "Height", "Width"]
		if icontribution==0:
			# 'elastic line :'
			for k in range(npars):
...
def interactive_define_ext_constrains(params_and_functions,constrains):
  ...
  if sys.platform == "win32":
	filename = os.path.join(tempfile.gettempdir(),"inputfitter")
  open(filename,"w").write(s)
  if sys.platform=="linux2":
	os.system("emacs  %s >/dev/null < /dev/null " % (filename),)
  elif sys.platform=="win32":
	os.popen2("start /wait notepad.exe  %s " %filename)
...
class Params_and_Functions:
	...
	def setParams(self, par_array):
		...
		self.NusedPar=0
		self.shapes=[]#List of contributions
...
# Main
#--------------------------------------------------------------------------------------------------------
def residual_fit(params, xdata, ydata, eps_data):
	global mod
	p = [params[k].value for k in params.keys()]
	simul = mod.Ft_I(p, xdata)
	return (ydata - simul)/eps_data
